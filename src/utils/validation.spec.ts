...
describe('testing the phone number validation ', () => {
    it('with valid format', () => {
      const test = Validation.isValidPhoneNumber('(11)91234-5678');
      expect(test).toBeTruthy();
    });
  
    it('with invalid format', () => {
      const test = Validation.isValidPhoneNumber('91234-5678');
      expect(test).toBeFalsy();
    });
  
    it('with invalid characters', () => {
      const test = Validation.isValidPhoneNumber('(11)91234abcd');
      expect(test).toBeFalsy();
    });
  
    it('with less than 11 digits', () => {
      const test = Validation.isValidPhoneNumber('(11)9123-5678');
      expect(test).toBeFalsy();
    });
  
    it('with more than 11 digits', () => {
      const test = Validation.isValidPhoneNumber('(11)912345-67890');
      expect(test).toBeFalsy();
    });
  
    it('with invalid DDD', () => {
      const test = Validation.isValidPhoneNumber('(99)91234-5678');
      expect(test).toBeFalsy();
    });
